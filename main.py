...
class ModelComparison:
    ...
    def plot_regression(self, y_pred_lin, y_pred_lasso, y_pred_bayesian, y_pred_ridge, y_pred_svr, y_pred_gpr):
        ...
        axs[2, 0].scatter(self.y, y_pred_svr, color='lightseagreen')
        m, b = numpy.polyfit(self.y.reshape((1,-1))[0], y_pred_svr.reshape((1,-1))[0], 1)
        axs[2, 0].plot(self.y, m*self.y+b, color='red')
        axs[2, 0].set_xlabel("Labels")
        axs[2, 0].set_ylabel("Predictions")
        axs[2, 0].set_title("SVR")

        axs[2, 1].scatter(self.y, y_pred_gpr, color='lightseagreen')
        m, b = numpy.polyfit(self.y.reshape((1,-1))[0], y_pred_gpr.reshape((1,-1))[0], 1)
        axs[2, 1].plot(self.y, m*self.y+b, color='red')
        axs[2, 1].set_xlabel("Labels")
        axs[2, 1].set_ylabel("Predictions")
        axs[2, 1].set_title("GPR")
...
if __name__ == "__main__":
    ...
    y_pred_gpr      = modelling.gpr()

    modelling.plot_regression(y_pred_lin, y_pred_lasso, y_pred_bayesian, y_pred_ridge, y_pred_svr, y_pred_gpr)
